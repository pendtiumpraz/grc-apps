...
type Subscription struct {
	BaseModel
	TenantID     string     `gorm:"not null" json:"tenant_id"`
	PlanType     string     `gorm:"not null;default:'basic'" json:"plan_type"` // basic, pro, enterprise
...
type Payment struct {
	...
	Amount          float64   `gorm:"not null" json:"amount"`
	Currency        string    `gorm:"default:'IDR'" json:"currency"`
	PaymentMethod   string    `gorm:"not null" json:"payment_method"` // bank_transfer, credit_card, etc
...
type SystemLog struct {
	BaseModel
	TenantID  string `json:"tenant_id"`                            // Optional - null for system-wide logs
	UserID    string `json:"user_id"`                              // Optional
	Level     string `gorm:"not null;default:'info'" json:"level"` // info, warning, error, critical
	Category  string `gorm:"not null" json:"category"`             // auth, api, system, security, billing
	...
	Duration  int    `json:"duration_ms"` // Request duration in milliseconds
}
